...
def mask_sources(
    ...
    if opening_iterations > 0:
        ...
        mask = ndi.binary_opening(mask, structure=struct, iterations=opening_iterations)
    ...
    if plot or outimage:
        ...
        ax[2].imshow(mask, interpolation="nearest", cmap="binary", origin="lower")
        ax[2].set_title("Mask")
        im_masked = im.copy()
        im_masked.mask_selection(mask)
        im_masked.plot(
            ax=ax[3], scale="linear", vmin=vmin, vmax=vmax, title="Masked image"
        )
