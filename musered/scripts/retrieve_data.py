...
@click.option('--calib', help='retrieve only calibrations for a list of '
...
def retrieve_data(mr, dataset, username, help_query, dry_run, force,
                  no_update_db, calib, ins_mode, start, end):
    ...
    for ds in dataset:
        ...
        table = Eso.query_instrument('muse', cache=False,
                                     column_filters=column_filters)
        if table is None:
            logger.warning('Found nothing')
            continue
        logger.info('Found %d files', len(table))
        table.keep_columns(['Object', 'Target Ra Dec', 'ProgId', 'DP.ID',
                            'EXPTIME [s]', 'DPR CATG', 'DPR TYPE', 'DPR TECH',
                            'TPL START', 'INS MODE', 'DIMM Seeing-avg'])
        print(table)
        if not dry_run:
