...
def qa_sky(mr, recipe_name=None, dates=None, skip=True, dry_run=False):
    ...
    rows = list(mr.reduced.find(recipe_name=recipe_name,
                                DPR_TYPE='SKY_SPECTRUM', name=dates))
    if skip:
...
def _psfrec(filename):
    ...
    return {
        ...
        'PR_betaB': beta[0], 'PR_betaV': beta[1], 'PR_betaR': beta[2]
    }


def _sky(filename):
    s = Table.read(filename)
    bands = [['skyB', 4850, 6000],
             ['skyV', 6000, 8000],
             ['skyR', 8000, 9300]]
    return {band: float(np.mean(s['data'][(s['lambda'] >= l1) &
                                          (s['lambda'] <= l2)]))
