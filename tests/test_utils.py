...
def test_parse_keywords(mr, caplog, tmpdir):
    ...
    testfile = os.path.join(CURDIR, "data", "MUSE.2017-06-16T01:34:56.867.fits")
...
def test_parse_weather(mr, caplog):
    ...
    assert (
        caplog.messages[1] == "File ./raw/MUSE.2017-06-16T01:34:56.867.NL.txt not found"
    )

    caplog.clear()
    os.makedirs("raw")
    shutil.copy(
        os.path.join(TESTDIR, "raw", "MUSE.2017-06-16T01:34:56.867.NL.txt"), "raw"
    )
    parse_weather_conditions(mr, force=True)
    assert caplog.messages == [
...
def test_parse_weather_invalid(mr, caplog):
    ...
    shutil.copy(
        os.path.join(TESTDIR, "raw", "MUSE.2017-06-16T01:37:47.867.invalid.txt"),
...
def test_qc_outliers(mr):
    t = find_outliers_qc_chan(
        mr, "qc_MASTER_BIAS", [f"QC_BIAS_MASTER{k}_RON" for k in range(1, 5)]
    )
    ...
    t = find_outliers_qc_chan(
        mr,
        "qc_MASTER_FLAT",
        [f"QC_FLAT_MASTER_SLICE{k}_MEAN" for k in range(1, 3)],
        nsigma=2,
    )
    assert len(t) == 7
    assert set(t["QC"]) == {"QC_FLAT_MASTER_SLICE1_MEAN", "QC_FLAT_MASTER_SLICE2_MEAN"}
